...
func Test_AssetEventRepository_CRUDAndOrdering(t *testing.T) {
	...
	older := &domain.AssetEvent{AssetID: asset.ID, Title: "Bought", Icon: "i-lucide-shopping-bag", EventDate: date(2025, 1, 1)}
	newer := &domain.AssetEvent{AssetID: asset.ID, Title: "Serviced", Icon: "i-lucide-wrench", EventDate: date(2026, 9, 8)}
	...
	description := "Replaced worn brushes"
	newer.Title, newer.Description = "Repaired", &description
	if err := repo.Update(ctx, org.ID, newer); err != nil {
		t.Fatalf("update event: %v", err)
	}
	fetched, err := repo.GetByID(ctx, org.ID, asset.ID, newer.ID)
	if err != nil || fetched == nil || fetched.Title != "Repaired" || fetched.Description == nil || *fetched.Description != description {
...
func Test_AssetEventRepository_ScopesEventsAndCascades(t *testing.T) {
	...
	asset, _ := fixtures.CreateAsset(ctx, org.ID, category.ID, "Drill")
	repo := NewAssetEventRepository(testDB.Pool)
	event := &domain.AssetEvent{AssetID: asset.ID, Title: "Serviced", Icon: "i-lucide-wrench", EventDate: date(2026, 9, 8)}
...
func Test_AssetEventRepository_CreateRejectsUnavailableAsset(t *testing.T) {
	...
	repo := NewAssetEventRepository(testDB.Pool)
	event := &domain.AssetEvent{AssetID: uuid.New(), Title: "Missing", Icon: "i-lucide-calendar", EventDate: date(2026, 9, 8)}
