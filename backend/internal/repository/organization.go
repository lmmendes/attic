...
func (r *OrganizationRepository) GetFeatures(ctx context.Context, orgID uuid.UUID) (*domain.OrganizationFeatures, error) {
	...
	err := r.pool.QueryRow(ctx, `
		SELECT locations, collections, tags, categories, attributes, conditions, warranties, plugins
		FROM organization_feature_settings WHERE organization_id = $1`, orgID).Scan(
		&features.Locations, &features.Collections, &features.Tags, &features.Categories, &features.Attributes,
		&features.Conditions, &features.Warranties, &features.Plugins,
	)
	if errors.Is(err, pgx.ErrNoRows) {
		*features = domain.OrganizationFeatures{Locations: true, Collections: true, Tags: true, Categories: true, Attributes: true, Conditions: true, Warranties: true, Plugins: true}
		return features, nil
	}
	if err != nil {
		return nil, err
...
func (r *OrganizationRepository) UpdateFeatures(ctx context.Context, orgID uuid.UUID, f *domain.OrganizationFeatures) error {
	_, err := r.pool.Exec(ctx, `
		INSERT INTO organization_feature_settings
			(organization_id, locations, collections, tags, categories, attributes, conditions, warranties, plugins)
		...
		f.Locations, f.Collections, f.Tags, f.Categories, f.Categories, f.Conditions, f.Warranties, f.Plugins)
	return err
}
