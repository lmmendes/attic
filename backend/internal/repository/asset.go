...
func (r *AssetRepository) GetByIDFull(ctx context.Context, id uuid.UUID) (*domain.Asset, error) {
	...
	if asset.CategoryID != nil {
		catQuery := `SELECT id, organization_id, parent_id, plugin_id, name, description, created_at, updated_at FROM categories WHERE id = $1`
		var cat domain.Category
		if err := r.pool.QueryRow(ctx, catQuery, *asset.CategoryID).Scan(
			&cat.ID, &cat.OrganizationID, &cat.ParentID, &cat.PluginID, &cat.Name, &cat.Description, &cat.CreatedAt, &cat.UpdatedAt,
		); err == nil {
			asset.Category = &cat
		}
	}
