...
const form = reactive({
  name: restoredDraft?.form.name || props.category?.name || '',
  description: restoredDraft?.form.description || props.category?.description || '',
  icon: restoredDraft?.form.icon || props.category?.icon || 'i-lucide-tag',
  parent_id: restoredDraft?.form.parent_id
    || props.category?.parent_id
    || (!isEditing.value && typeof route.query.parent_id === 'string' ? route.query.parent_id : undefined)
})

// Attribute selection
const selectedAttributes = ref<AttributeSelection[]>(
  restoredDraft?.selectedAttributes.map(attribute => ({ ...attribute }))
  || props.category?.attributes?.map((attribute, index) => ({
...
async function saveCategory() {
  ...
  saving.value = true
  try {
    await apiFetch(isEditing.value ? `/api/categories/${props.category?.id}` : '/api/categories', {
    ...
    clearCategories()
    toast.add({ title: `Category ${isEditing.value ? 'updated' : 'created'} successfully`, color: 'success' })
    if (!isEditing.value) categoryDraft.value = null
    router.push('/categories')
  } catch {
    toast.add({ title: `Failed to ${isEditing.value ? 'update' : 'create'} category`, color: 'error' })
  } finally {
    saving.value = false
  }
}
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-6">
      <div class="flex flex-col gap-2">
        <nav class="flex items-center text-sm font-medium text-muted">
          ...
          <span class="mx-2 text-mist-300 dark:text-mist-600">/</span>
          <span class="text-mist-950 dark:text-white">{{ isEditing ? 'Edit' : 'New' }}</span>
        </nav>
        <div>
          ...
          <p class="text-muted mt-1">
            {{ isEditing ? 'Update the structure for this group of assets.' : 'Define the structure for a new group of assets.' }}
          </p>
        </div>
      </div>
